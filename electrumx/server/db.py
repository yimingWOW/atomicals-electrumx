...
class DB:
    ...
    def get_utxo_set(self, limit, offset):
        utxo_set = []
        atomical_active_location_key_prefix = b'a'
        for atomical_active_location_key, atomical_active_location_value in self.utxo_db.iterator(prefix=atomical_active_location_key_prefix):
            if atomical_active_location_value:
                atomical_id = atomical_active_location_key[1 : 1 + ATOMICAL_ID_LEN]
                location = atomical_active_location_key[1 + ATOMICAL_ID_LEN : 1 + ATOMICAL_ID_LEN + ATOMICAL_ID_LEN]
                location_value, = unpack_le_uint64(atomical_active_location_value[HASHX_LEN + SCRIPTHASH_LEN : HASHX_LEN + SCRIPTHASH_LEN + 8])
                utxo_set.append({
                    'id': location_id_bytes_to_compact(atomical_id),
                    'loc': location_id_bytes_to_compact(location),
                    'v': location_value
                })
        return utxo_set
    ...
    def get_txos(self, limit, offset):
        ...
        for location_key, location_result_value in self.utxo_db.iterator(prefix=i_prefix):
            location_id = location_key[1 : 1 + ATOMICAL_ID_LEN]
            atomical_id = location_key[1 + ATOMICAL_ID_LEN : 1 + ATOMICAL_ID_LEN + ATOMICAL_ID_LEN]
            location_value, = unpack_le_uint64(atomical_active_location_value[HASHX_LEN + SCRIPTHASH_LEN : HASHX_LEN + SCRIPTHASH_LEN + 8])
            txos_list.append({
                'id': location_id_bytes_to_compact(atomical_id),
                'loc': location_id_bytes_to_compact(location_id),
                'v': location_value
            })
        return txos_list
